...
const gorraVerde = new Producto("GORRA VERDE", "./assets/gorraVerde.jpg", "Gorra", 600, "Verde")
const pilusoBlanco = new Producto("PILUSO BLANCO", "./assets/pilusoBlanco.jpg", "Piluso", 1500, "Blanco");
const gorraNegra = new Producto("GORRA NEGRA", "./assets/gorra.jpg", "Gorra", 800, "Negro");
const sombreroNegro = new Producto("SOMBRERO NEGRO", "./assets/sombreroNegro.jpg", "Sombrero", 2000, "Negro");
...
function productsInit() {
    ...
    <div id="products_wrapper">
        <div id="filters">
            ...
            <ul class="ul_filters">
                ...
                <li class="item"><input id="gorra_filter" name="filter" type="radio" data-tipo="Gorra"> Gorra</li>
                <li class="item"><input id="piluso_filter" name="filter" type="radio" data-tipo="Piluso"> Pilusos</li>
                <li class="item"><input id="sombrero_filter" name="filter" type="radio" data-tipo="Sombrero"> Sombrero</li>
              </ul>
              <div>
  ...
  const toggle = document.querySelector('.toggle'); 
	...
	const arrayObjects = [gorraVerde, pilusoBlanco, gorraNegra, sombreroNegro];
  ...
  // TIPO
  let gorraFilter = document.getElementById("gorra_filter");
  gorraFilter.addEventListener("click", gorraFilterClothes);
  let pilusoFilter = document.getElementById("piluso_filter");
  pilusoFilter.addEventListener("click", pilusoFilterClothes);
  let sombreroFilter = document.getElementById("sombrero_filter");
  sombreroFilter.addEventListener("click", sombreroFilterClothes);
  ...
  function gorraFilterClothes() {
    products.innerHTML = ``;
    const filterObjets = arrayObjects.map((element) => {
      if (element.type === gorraFilter.dataset.tipo) {
        renderProducts(element);
        popUp(element);
      };
    });
  ...
  function pilusoFilterClothes() {
    products.innerHTML = ``;
    const filterObjets = arrayObjects.map((element) => {
      if (element.type === pilusoFilter.dataset.tipo) {
        renderProducts(element);
        popUp(element);
      };
    });
  ...
  function sombreroFilterClothes() {
    products.innerHTML = ``;
    const filterObjets = arrayObjects.map((element) => {
      if (element.type === sombreroFilter.dataset.tipo) {
        renderProducts(element);
        popUp(element);
      };
    });
